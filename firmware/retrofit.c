...
static void wheel_speed_update(void)
{
    ...
    for (i = 0; i < 4; i++) {
        ...
        if (p == 0 || (ms() - 0) > 0) {  /* stale handled by main loop reset */
            wheel_speed_mps[i] = p ? (c_per_pulse * 1000000.0f / p) : 0.0f;
        }
        if (p == 0) wheel_speed_mps[i] = 0.0f;
    }
}
...
static float adc_current_a(void)  /* AD5/AD6 via ADS1115 (iic.c), placeholder */
{
    extern float ads_read_ad5_current(void);  /* wire to iic.c ADS1115 read */
    return ads_read_ad5_current();
}
...
static void stepper_update(void)
{
    ...
    if (!estop_latched && err_mm != 0.0f) {
        ...
        float hz = err_mm * STEPPER_STEPS_PER_MM * 20.0f; /* 20 mm/s max */
        if (hz < 0) hz = -hz;
...
void retrofit_init(void)
{
    ...
    g.GPIO_Pin = GPIO_Pin_6 | GPIO_Pin_7 | GPIO_Pin_0 | GPIO_Pin_1;
    g.GPIO_PuPd = GPIO_PuPd_NOPULL;
