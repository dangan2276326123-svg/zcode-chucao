...
拒绝条件（任一即拒，抛 IpmRejected）：
  ...
  * H 退化：非 3x3 / 含非有限值 / 秩 < 3 / |det| ≈ 0
    （09-21 复审：只看 h22 会放过 diag(1,0,1) 这种 h22=1 却不可逆的矩阵）
  * reproj_mean_m 为负或非有限；lookahead 非 0 < near < far ≤ 10 m 且有限；
    far_m 换算成像素后超出 BEV 画布——"矩阵尺寸对但采样区在图外"也是废标定

坐标约定（与 pc/perception.py 现有 BEV 网格一致）：
  地面系：X 向右为正（m），Y 向前为正（m），原点在相机正下方地面。
  BEV 像素：x_bev = X*s + W/2，y_bev = H_img - Y*s，s = scale_px_per_m。
  本实现**选**各向同性（横纵向同一个 m/px），这是接口约定而不是物理定律——
  各向异性的 BEV 同样可以合法，只是行墙/宽度判据要分开算。旧占位的
  0.0015/0.0030 不是"标定出的各向异性"，是没标定过随手填的数。
  注意 `scale_px_per_m` 是**我们给 BEV 绘图挑的分辨率**，不是标定发现的常数：
  5 cm 在 s=100 下是 5 px，在 s=0.0015 m/px 的旧口径下是 33.3 px，物理距离相同。
  所以换真标定**不需要把米制阈值按比例缩放**，需要的是重新验证控制效果。
"""
import json
import os
